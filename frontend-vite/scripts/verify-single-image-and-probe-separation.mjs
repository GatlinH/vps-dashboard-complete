...
assert.match(detail, /PING 延迟[\s\S]*?targetCount/);
assert.match(detail, /全球 VPS 探针延迟[\s\S]*?当前节点 → 其它 VPS/);
assert.match(main, /暂无节点侧互探采样/);
assert.match(main, /等待当前 VPS Agent 上报/);
assert.match(main, /全球探针 = 当前节点 → 其它节点/);
