...
import { buildServerPayload } from './serverPayload.js';

export { buildServerPayload } from './serverPayload.js';

/** 后端字段 → 前端字段映射 */
function normalize(s) {
  const cfg = s.agent_config && typeof s.agent_config === 'object' ? s.agent_config : {};
  const meta =
    cfg.inventory_meta && typeof cfg.inventory_meta === 'object' ? cfg.inventory_meta : {};
  return {
    ...s,
    agent_config: cfg,
    ...
    provider: s.provider || '',
    provider_guess:
      s.provider_guess || meta.provider_guess || cfg.provider_guess || meta.org || meta.isp || '',
...
export async function createServer(payload) {
  const body = buildServerPayload(payload, { forCreate: true });
...
export async function updateServer(id, payload) {
  const body = buildServerPayload(payload);
