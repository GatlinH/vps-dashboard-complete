...
export function updateEngineFX(host, time) {
  if (host.exhaustGroup) {
    if (host.exhaustGroup.parent === host.ship || host.exhaustGroup.parent === host.anchor) {
      host.exhaustGroup.position.set(0, 0, 0);
      host.exhaustGroup.rotation.set(0, 0, 0);
      host.exhaustGroup.scale.set(1, 1, 1);
    } else {
      host.exhaustGroup.position.copy(host.anchor.position);
      host.exhaustGroup.rotation.copy(host.anchor.rotation);
      host.exhaustGroup.scale.copy(host.anchor.scale);
    }
    const propulsionPulse = 0.5 + 0.5 * Math.sin(time * 4.0);
    host.exhaustCoreMaterials?.forEach((mat) => { mat.opacity = 0.76 + propulsionPulse * 0.18; });
    host.exhaustTailMaterials?.forEach((mat) => {
      if (mat?.uniforms?.uTime) mat.uniforms.uTime.value = time;
      if ('opacity' in (mat || {})) mat.opacity = 0.46 + propulsionPulse * 0.20;
    });
    host.plasmaFilamentMaterials?.forEach((mat) => { if (mat?.uniforms?.uTime) mat.uniforms.uTime.value = time; });
    host.particlePlumeMaterials?.forEach((mat) => { if (mat?.uniforms?.uTime) mat.uniforms.uTime.value = time; });
    host.exhaustGroup.userData.liveLayerCounts = Object.fromEntries(Object.entries(host.exhaustNiagaraLayers || {}).map(([key, group]) => [key, group.children.length]));
    host._dbgSet('starshipPropulsionPulse', Number(propulsionPulse.toFixed(3)));
  }
  const coilPulse = 0.50 + 0.50 * Math.sin(time * 3.2);
  ...
  host._dbgSet('starshipWarpPulse', Number(coilPulse.toFixed(3)));
}

export const engineFXMethods = {
