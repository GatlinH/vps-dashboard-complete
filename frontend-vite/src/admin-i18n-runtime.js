...
Object.assign(ZH_TO_EN, {
  '管理站点图片、纹理、页脚与展示宽度；支持站内路径（如 /assets/custom/logo.png）或 https 外链；图片路径保存后由对应页面按需读取。':'Manage site images, textures, footer, and layout width. Supports site paths such as /assets/custom/logo.png or HTTPS URLs. Saved image paths are used by the relevant pages.',
  '展示设置':'Display Settings',
  '在网站中显示IP标签':'Show IP labels on site',
  '启用后在服务器列表中显示 IPv4 / IPv6 标签；需在设置中启用相关 IP 地址检测功能。':'Show IPv4 / IPv6 labels in the server list. Requires IP detection to be enabled in Settings.',
  '在详情页显示服务器列表':'Show server list on detail pages',
  '启用后在服务器详情页面显示服务器列表，方便用户返回首页或切换到其他服务器。':'Show the server list on detail pages so users can return or switch servers quickly.',
  '背景图片路径/URL（桌面端）':'Desktop background image path / URL',
  '设置桌面端首页的自定义背景图片。':'Custom background image for the desktop homepage.',
  '背景图片路径/URL（移动端）':'Mobile background image path / URL',
  '设置移动端首页的自定义背景图片。留空时使用桌面端背景。':'Custom background image for mobile. Leave blank to use the desktop background.',
  '图片位置自定义':'Custom Image Slots',
  '登录页背景图片路径/URL':'Login background image path / URL',
  '覆盖后台/前台登录场景背景。':'Overrides the admin/frontend login background.',
  '星空/星云背景图片路径/URL':'Starmap / nebula background path / URL',
  '覆盖星图/后台空间底图。':'Overrides the starmap and admin space background.',
  'Logo / 站点标识图片路径/URL':'Logo / site mark image path / URL',
  '覆盖侧栏、登录与站点品牌图。':'Overrides sidebar, login, and site branding images.',
  'Favicon 图标路径/URL':'Favicon path / URL',
  '覆盖浏览器标签页与快捷图标。':'Overrides browser tab and shortcut icons.',
  '地球纹理图片路径/URL':'Earth texture image path / URL',
  '可选：覆盖地球底图纹理（留空使用默认真实影像）。':'Optional: overrides the earth base texture. Leave blank to use the default satellite imagery.',
  '云层纹理图片路径/URL':'Cloud texture image path / URL',
  '可选：覆盖地球云层纹理。':'Optional: overrides the earth cloud texture.',
  '飞船/装饰模型图片路径/URL':'Hero / decoration image path / URL',
  '覆盖首页飞船、装饰或插画图位。':'Overrides the homepage hero, decoration, or illustration image.',
  '默认节点图标路径/URL':'Default node icon path / URL',
  '覆盖 VPS 节点 marker / 默认图标。':'Overrides the VPS node marker / default icon.',
  '商城节点位置':'Marketplace node position',
  '最前：商城节点显示在前面；保持：按权重排序，不区分在线/离线；最后：商城节点显示在最后。':'First: show marketplace nodes first. Keep: sort by weight regardless of status. Last: show marketplace nodes last.',
  '最前的':'First',
  '保持':'Keep',
  '最后的':'Last',
  '自定义页脚HTML':'Custom footer HTML',
  '设置自定义页脚 HTML 内容，以替换网站页脚和授权信息。':'Set custom footer HTML to replace the site footer and attribution.',
  '主要内容宽度':'Main content width',
  '调整主要内容最大宽度，单位为视口宽度百分比 VW；设置为 100 表示占满宽度。':'Adjust the maximum content width in VW. Set to 100 to use full width.',
  'API 密钥':'API Key',
  'API 密钥可访问并操作后台资源，包括修改设置和账号密码等；请谨慎保管。留空表示关闭 API 密钥功能。':'API keys can access and modify admin resources, including settings and accounts. Keep them safe. Leave blank to disable API keys.',
  '团队域名 team_domain *':'Team domain team_domain *',
  '策略 AUD policy_aud *':'Policy AUD policy_aud *'
});
