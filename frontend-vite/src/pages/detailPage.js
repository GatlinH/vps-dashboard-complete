...
export function renderDetailConsole(ctx) {
  ...
  const vpsProbeCount = (vpsProbeTargetsData?.targets || []).filter((target) => String(target?.key || '').startsWith('vps-')).length;
  ...
  return `
    <section class="fleet-detail-console">
      ...
      <main class="fleet-chart-matrix">
        ...
        <div class="fleet-chart-card compact-metric-card ping-multi-card"><div class="fleet-chart-head"><span>全球 VPS 探针延迟 · ${historyLabel}</span><strong>${vpsProbeCount} VPS</strong></div><canvas id="detailGlobalVpsProbeChart"></canvas></div>
      ...
      <section class="fleet-right-zone">
        <div class="fleet-probe-grid">
          ...
          <div class="fleet-panel fleet-global-vps-probe-table-panel">
            <div class="fleet-title">全球 VPS 探针延迟</div>
            <table class="fleet-table compact"><thead><tr><th>VPS 目标</th><th>ms</th><th data-i18n="loss">${t('loss')} %</th><th>链路</th></tr></thead><tbody>${h.renderGlobalVpsProbeRows(vpsProbeTargetsData)}</tbody></table>
          </div>
        </div>
      </section>
