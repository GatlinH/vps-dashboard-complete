  ...
  return `
    <section class="fleet-detail-console">
      ...
      <main class="fleet-chart-matrix">
        ...
        <div class="fleet-chart-card compact-metric-card ping-multi-card"><div class="fleet-chart-head"><span>PING 延迟 · ${pingWindowLabel} · 掉线留空</span><strong class="detail-ping-target-count">${targetCount} 目标</strong></div><canvas id="detailPingChart"></canvas></div>
        ...
        <div class="fleet-chart-card pseudo process-count-card compact-metric-card resource-mini-card"><div class="fleet-chart-head"><span>运行进程数 · ${resourceWindowLabel}</span><strong>${processMeta.countText}</strong></div><div class="process-count-meta"><span>${processMeta.count == null ? '等待 Agent 上报进程数' : '仅统计总数，不采集进程名称或命令行'}</span><span class="process-count-latest">${processMeta.count == null ? '无可用样本' : '主机级监控'}</span></div><canvas id="detailProcessCountChart"></canvas></div>
      </main>
