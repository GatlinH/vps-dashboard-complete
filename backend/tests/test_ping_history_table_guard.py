...
def test_ping_history_table_ddl_runs_only_once_per_process(app, monkeypatch):
    ...
    calls = []
    original_execute = probe.db.session.execute
    ...
    with app.app_context():
        probe._PTR_HISTORY_TABLE_READY = False
        monkeypatch.setattr(probe.db.session, 'execute', tracked_execute)
        assert probe._target_history_table_ready() is True
        assert probe._target_history_table_ready() is True
