...
def test_update_product_requires_auth(client, auth_headers):
    ...
    # 先创建一个商品
    create_resp = client.post('/api/aff/', json={
        'provider': 'UpdateTest',
        'price': 1.0,
    }, headers=auth_headers)
    ...
    # 无认证更新应返回 401
    resp = client.put(f'/api/aff/{pid}', json={'price': 2.0})
    assert resp.status_code == 401
...
def test_update_product_with_admin(client, auth_headers):
    ...
    resp = client.put(f'/api/aff/{pid}', json={
...
def test_delete_product_requires_auth(client, auth_headers):
    ...
    resp = client.delete(f'/api/aff/{pid}')
    assert resp.status_code == 401


def test_list_products_with_stock_filter(client, auth_headers):
    """GET /api/aff/?stock=avail 过滤可购商品"""
    client.post('/api/aff/', json={
        'provider': 'StockTest',
        'stock': 'avail',
        'price': 1.0,
    }, headers=auth_headers)

    resp = client.get('/api/aff/?stock=avail')
    assert resp.status_code == 200
    data = resp.get_json()
