...
from models.models import AuditLog, OpsEvent, db

_ALLOWED = {AuditLog: 'audit_logs', OpsEvent: 'ops_events'}
...
def cleanup_audit_and_ops(app, retention_days=14, batch_size=1000, dry_run=False):
    ...
    result = {'cutoff': cutoff, 'audit_logs': 0, 'ops_events': 0, 'dry_run': bool(dry_run)}
    if dry_run:
    ...
    with app.app_context():
        for model, key in _ALLOWED.items():
            result[key] = _bulk_delete(model, cutoff, size)
    return result
