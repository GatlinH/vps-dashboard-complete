...
def _public_view_json(view, sid, query_string=""):
    """Execute an existing public view under its own query-string contract."""
...
def get_public_server_detail(sid):
    ...
    from api.probe import public_ping_targets, public_ping_targets_history
    from api.traffic import _build_traffic_payload

    hours_query = "&hours=6" if days == 0 else ""
    history_query = f"days={history_days}&limit={limit}&bucket_minutes={bucket_minutes}{hours_query}"
    ping_hours = 6 if days == 0 else max(1, days * 24)
    payload = {
        "server": server.to_dict(public_only=True),
        "live": _public_view_json(get_public_live_snapshot, sid).get("live", {}),
        "history": _public_view_json(get_public_history, sid, history_query),
        "resource_timeline": _public_view_json(
            get_public_history, sid, "days=1&limit=900&metric=resource_timeline"
        ).get("data", []),
        "process_history": _public_view_json(
            get_public_history, sid, "days=1&limit=720&metric=process_count"
        ).get("data", []),
        "traffic": _build_traffic_payload(server),
        "ping_targets": _public_view_json(public_ping_targets, sid, "count=1"),
        "ping_history": _public_view_json(
            public_ping_targets_history, sid, f"hours={ping_hours}&limit={limit}"
        ),
    }
    try:
        extensions.redis_client.setex(
...
def get_public_history(sid):
    ...
    if metric == "resource_timeline":
        # Agent telemetry shares ProbeResult with scheduler/network probe rows.
        # Filter resource-null rows BEFORE applying the point budget: otherwise
        # recent latency-only rows consume `limit` and a declared one-hour CPU/RAM
        # chart receives just a few minutes of values at its right edge.
        since = datetime.now(timezone.utc) - timedelta(hours=1)
        resource_filter = or_(ProbeResult.cpu_use.isnot(None), ProbeResult.ram_use.isnot(None))
        total = (ProbeResult.query
                 .filter(ProbeResult.server_id == sid, ProbeResult.created_at >= since, resource_filter)
                 .count())
        # Limit is intentionally bounded for the raw 5s detail series. Fetch in
        # DESC order (index-friendly newest-N) then reverse to preserve the API's
        # old-to-new chart contract.
        ...
        for row in newest:
            ...
            data.append(item)
        return jsonify(
            data=data, total=total, count=len(data), metric="resource_timeline",
            hours=1, history_source="raw", bucketed=False, bucket_minutes=0,
        )
    if metric == "network_timeline":
        # Network chart has its own six-hour contract. Filtering before GROUP BY
        # prevents CPU-only telemetry and scheduler rows from creating null buckets
        # and avoids scanning the selected 1-90d detail range on first paint.
        hours = 6
        bucket_minutes = 3
    ...
    history_hours = request.args.get("hours", type=float)
