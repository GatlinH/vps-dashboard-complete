...
浏览器 / 移动端
   │
   ├─ http://<IP>:4500/              公开首页
   ├─ http://<IP>:4500/?server=<id>  VPS 详情页
   └─ http://<IP>:4500/admin.html    管理后台
          │
          ▼
Dashboard 应用镜像 :5000（宿主机发布 :4500；内含 Flask API 与 Vite 静态资源）
   ├─ MySQL 8
   ├─ Redis 7
   ├─ Agent consumer
   └─ Watchtower 手动镜像更新

宿主机 vps-agent.service → http://127.0.0.1:5000/api/v1/agent/push
```

Caddy、Nginx 或其他 TLS 反向代理完全可选，且由部署者自行管理。参见 `docs/optional-reverse-proxy.md`。

## 默认镜像

| 服务 | 镜像 |
|---|---|
| Dashboard / schema_init / agent_consumer | `ghcr.io/gatlinh/vps-dashboard-complete-backend:latest`（内含 Vite 前端） |
| MySQL | `mysql:8.0.33` |
...
每次 push 到 `main` 后会跑 CI，并发布一个内含前端静态资源的 Dashboard 镜像到 GHCR。
...
6. 发布 Dashboard 直连端口 `0.0.0.0:4500`；
7. 安装宿主机只读 Agent 并注册为主控节点。

安装程序不会安装或修改 Caddy、Nginx 或其他系统代理配置；TLS 反向代理为用户可选配置。
...
## Windows（Docker Desktop + Master Agent 服务）

Windows 10/11 或 Windows Server 可运行 Dashboard 容器，并以 Windows Service 运行只读 Master Agent；Docker Desktop 必须切换到 **Linux containers**。Linux 的 `install.sh` / `update.sh` 与 systemd 流程保持不变。
...
Agent 配置保存于 `%ProgramData%\VpsDashboardAgent\agent.env`，ACL 仅授予 `SYSTEM` 与 `Administrators`；Windows 默认不启用 Watchtower。详见 [`docs/WINDOWS_DEPLOYMENT.md`](docs/WINDOWS_DEPLOYMENT.md)。
