...
- **移动端体验**：详情页资源卡片自适应换行，网络吞吐折线从 0 基线起步，3D 星球与节点标签针对手机做避让和可读性优化。
- **管理员后台**：服务器管理、用户/角色、登录设置、通知规则、站点外观、GeoIP、备份和审计日志。
- **Agent 采集**：安装脚本自动注册 Agent，定时回传系统指标、运行环境、网络指标和探针结果。
...
- 移动端详情页资源卡片改为自适应布局，避免 CPU/内存/磁盘/负载内容裁切。
- 网络吞吐图移动端使用平滑折线，首点从坐标系 0 基线起步，tooltip 保留真实采样值。
- 详情页“节点遥测”改为运行环境卡片，展示操作系统、内核族、硬件架构和 CPU 类型。
- 匿名公开接口会对内核版本与 CPU 型号做粗粒度脱敏，降低公开指纹识别风险。
- 初装未配置 Ping 目标时默认不使用外部预设目标，接口返回明确的空目标结构。
